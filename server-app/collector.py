...
DEFAULT_FMW_PATCH_BASELINES = {
    "12c": {
        "family": "12c",
        "version": "12.2.1.4.0",
        "latestRelease": "September 2026 12.2.1.4.260908 SPB",
        "latestPatch": _patch_row("IDM STACK PATCH BUNDLE 12.2.1.4.260908", "39990616", ALL_IDM_COMPONENTS),
        "patches": [
            _patch_row("MERGE REQUEST ON TOP OF 12.2.1.4.0 FOR BUGS 39465265 39301239", "39726402", ALL_IDM_COMPONENTS),
            _patch_row("OIM 12CPS3 UPGRADE STUCK AT SCHEMAREADINESSQUERIES QUERYINDEXNAMES", "32999272", ALL_IDM_COMPONENTS),
            _patch_row("Fix for Bug 39425385", "39425385", ["OID"]),
            _patch_row("PERL SECURITY PATCH UPDATE 12.2.1.4.260120", "38590502", ["OID"]),
            _patch_row("ONE OFF PATCH TO RELINK OID WITH DBCLIENT - JUL'26 BP", "39559610", ["OID"]),
            ...
            _patch_row("IPF PATCH 12.2.1.4.260818", "39810314", ["OAM", "OIG"]),
            _patch_row("FMW PLATFORM BUNDLE PATCH 12.2.1.4.240812", "36789759", ALL_IDM_COMPONENTS),
            _patch_row("RDA release 26.3.2026915 for FMW 12.2.1.4.0", "39798204", ALL_IDM_COMPONENTS),
            _patch_row("FMW THIRDPARTY BUNDLE PATCH 12.2.1.4.260617", "39574504", ALL_IDM_COMPONENTS),
            _patch_row("WEBLOGIC SAMPLES SPU 12.2.1.4.240416", "36426672", ALL_IDM_COMPONENTS),
            _patch_row("JDBC19.31 BUNDLE PATCH 12.2.1.4.260603", "39487427", ALL_IDM_COMPONENTS),
            _patch_row("OCT 2024 CLONING SPU FOR FMW 12.2.1.4.0", "37056593", ALL_IDM_COMPONENTS),
            _patch_row("Coherence 12.2.1.4 Cumulative Patch 32 (12.2.1.4.32)", "39864526", ALL_IDM_COMPONENTS),
            _patch_row("WLS PATCH SET UPDATE 12.2.1.4.260823", "39919567", ALL_IDM_COMPONENTS),
            _patch_row("ADR FOR WEBLOGIC SERVER 12.2.1.4.0 - SIZE OPTIMIZED FOR JAN 2024", "35965629", ALL_IDM_COMPONENTS),
            _patch_row("OSS 19C BUNDLE PATCH 12.2.1.4.260616", "39567777", ALL_IDM_COMPONENTS),
            _patch_row("OPSS Bundle Patch 12.2.1.4.260904", "39979941", ALL_IDM_COMPONENTS),
            _patch_row("OWSM BUNDLE PATCH 12.2.1.4.260818", "39896168", ALL_IDM_COMPONENTS),
            _patch_row("OIM BUNDLE PATCH 12.2.1.4.260818", "39895193", ["OAM", "OIG"]),
            _patch_row("OUD BUNDLE PATCH 12.2.1.4.260721 - AUG BP", "39761493", ["OUD"]),
            _patch_row("ADF BUNDLE PATCH 12.2.1.4.260825", "39930279", ALL_IDM_COMPONENTS),
            _patch_row("SOA Bundle Patch 12.2.1.4.260825", "39928046", ["OIG"]),
            _patch_row("WebCenter Core Bundle Patch 12.2.1.4.260818", "39901894", ALL_IDM_COMPONENTS),
            _patch_row("OAM BUNDLE PATCH 12.2.1.4.260821", "39912438", ["OAM", "OIG"]),
            _patch_row("OAMCLIENT BUNDLE PATCH 12.2.1.4.260819", "39913319", ["OAM", "OIG"]),
            _patch_row("FMWCONTROL Bundle Patch 12.2.1.4.260803", "39825967", ALL_IDM_COMPONENTS),
            _patch_row("UMS (On Prem) Bundle Patch 12.2.1.3.260827", "39939504", ALL_IDM_COMPONENTS),
            _patch_row("OID BUNDLE PATCH 12.2.1.4.260814", "39882405", ["OID"]),
        ],
    },
    "14c": {
        "family": "14c",
        "version": "14.1.2.1",
        "latestRelease": "September 2026 14.1.2.1.260906 SPB",
        "latestPatch": _patch_row("IDM STACK PATCH BUNDLE 14.1.2.1.260906", "39983554", ALL_IDM_COMPONENTS),
        "patches": [
            _patch_row("MERGE REQUEST ON TOP OF 14.1.2.0.0 FOR BUGS 39458344 39301239", "39726399", ALL_IDM_COMPONENTS),
            _patch_row("Unable to Retrieve Request Details by REST Client With End User - Error occurred in routing slip default/DefaultOperationalApproval!5.0/ApprovalTask with message: null.", "37512243", ["OIG"]),
            _patch_row("OINAV PATCH 14.1.2.0.260421", "39019346", ["OAM", "OIG"]),
            _patch_row("IPF PATCH 14.1.2.1.260818", "39810320", ["OAM", "OIG"]),
            ...
            _patch_row("JDBC23.26.2 BUNDLE PATCH 14.1.2.0.260603", "39487373", ALL_IDM_COMPONENTS),
            _patch_row("DATABASE RELEASE UPDATE 23.26.2.0.0 FOR FMW DBCLIENT", "39264447", ["OAM", "OIG", "OID"]),
            _patch_row("STACK:FMW:RCU prereq steps fails while trying to validate the DB connection", "38391376", ALL_IDM_COMPONENTS),
            _patch_row("RDA release 26.3.2026915 for FMW 14.1.2.0.0", "39798193", ALL_IDM_COMPONENTS),
            _patch_row("FMW Thirdparty Bundle Patch 14.1.2.0.260821", "39916998", ALL_IDM_COMPONENTS),
            _patch_row("WLS PATCH SET UPDATE 14.1.2.0.260823", "39919611", ALL_IDM_COMPONENTS),
            _patch_row("WEBLOGIC SAMPLES PATCH 14.1.2.0.250715", "38063775", ALL_IDM_COMPONENTS),
            _patch_row("Coherence 14.1.2.0 Cumulative Patch 9 (14.1.2.0.9)", "39864483", ALL_IDM_COMPONENTS),
            _patch_row("OWSM BUNDLE PATCH 14.1.2.0.260818", "39895617", ALL_IDM_COMPONENTS),
            _patch_row("OAM BUNDLE PATCH 14.1.2.1.260820", "39908881", ["OAM", "OIG"]),
            _patch_row("OIM BUNDLE PATCH 14.1.2.1.260819", "39904932", ["OAM", "OIG"]),
            _patch_row("SOA Bundle Patch 14.1.2.0.260820", "39913274", ["OIG"]),
            _patch_row("OUD BUNDLE PATCH 14.1.2.1.260721 - AUG BP", "39761325", ["OUD"]),
            _patch_row("ADF BUNDLE PATCH 14.1.2.0.260817", "39889965", ALL_IDM_COMPONENTS),
            _patch_row("WebCenter Core Bundle Patch 14.1.2.0.260818", "39901669", ALL_IDM_COMPONENTS),
            _patch_row("UMS Bundle Patch 14.1.2.0.260827", "39940047", ALL_IDM_COMPONENTS),
            _patch_row("OPSS Bundle Patch 14.1.2.0.260904", "39979746", ALL_IDM_COMPONENTS),
            _patch_row("FMWCONTROL BUNDLE PATCH 14.1.2.0.260723", "39777647", ALL_IDM_COMPONENTS),
            _patch_row("OID BUNDLE PATCH 14.1.2.1.0(ID:260814.2147)", "39884090", ["OID"]),
            _patch_row("OAMCLIENT BUNDLE PATCH 14.1.2.0.260817", "39894980", ["OAM", "OIG"]),
        ],
    },
}
...
def build_oaa_schema_host_sqlplus_command(sql_arg, sql_text, oracle_home_hint=""):
    template = (
        ...
        "  resolved_sqlplus=$(readlink -f \"$sqlplus_bin\" 2>/dev/null || printf '%s\\n' \"$sqlplus_bin\")\n"
        "  [ -n \"$resolved_sqlplus\" ] && sqlplus_bin=\"$resolved_sqlplus\"\n"
        "  sqlplus_dir=$(dirname \"$sqlplus_bin\")\n"
        "  if [ ! -d \"$ORACLE_HOME/sqlplus/mesg\" ]; then\n"
        "    case \"$sqlplus_dir\" in\n"
        "      */bin)\n"
        "        derived_oracle_home=$(cd \"$sqlplus_dir/..\" 2>/dev/null && pwd -P)\n"
        "        if [ -n \"$derived_oracle_home\" ] && [ -d \"$derived_oracle_home/sqlplus/mesg\" ]; then\n"
        "          export ORACLE_HOME=\"$derived_oracle_home\"\n"
        "        fi\n"
        "        ;;\n"
        "    esac\n"
        "  fi\n"
        ...
        "    sqlplus_lib_path=\"$ORACLE_HOME/lib:$sqlplus_dir\"\n"
        "  else\n"
        "    sqlplus_lib_path=\"$sqlplus_dir\"\n"
        "  fi\n"
        "  if [ -n \"$LD_LIBRARY_PATH\" ]; then\n"
        "    export LD_LIBRARY_PATH=\"$sqlplus_lib_path:$LD_LIBRARY_PATH\"\n"
        "  else\n"
        "    export LD_LIBRARY_PATH=\"$sqlplus_lib_path\"\n"
        "  fi\n"
